...
public class Replicate {
	...
	public void setSpecCountThreshold(int thresh){
		...
		//That is, spec counts that are less than this threshold will not be normalized
	...
	//Searches the header of the file for the exact column name in the following format:
	//			CRxFy for control and DRxFy for drug
	public void setOrder(String[] specHeader, int _numFracs){
		numFracs = _numFracs;
	...
	public void normalizeSpecCountAcrossFractions(){
		...
		for(String proteinID: proteinIDSet){
			...
			for(int i = 0; i< numFracs; i++){
				if(sum[i] > 0.5){ // excludes 0's. sum should be a whole number
	...
	public void normalizeSpecCountAcrossProteinID(){
		...
		for(String proteinID: ProteinIDtoSpecCountVectors.keySet()){
			...
			if(sum > (specCountThreshold+DOUBLE_BUFFER)){ // bigger than 5. so 6 and up. 
				for(int i = 0; i < SpecCountVector.size(); i++){
